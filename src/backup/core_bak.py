        ...
        while not backup_success:
            ...
            if process.returncode != 0:
                ...
                table_matches = re.findall(
                    r"Table '([\w\.]+)' doesn't exist", stderr)
                ...
                if table_matches:
                    ...
                    if table_name_lower not in skipped_tables:
                        ...
                        logger.warning(f"[{config_name}] 检测到缺失表: {table_name}")

                        # 添加--ignore-table参数
                        cmd += [f"--ignore-table={table_name_lower}"]
                        logger.info(
                            f"[{config_name}] 添加忽略参数: --ignore-table={table_name_lower}")
            ...
            # 记录当前错误信息
            if process.returncode != 0:
                attempt_info = f"{len(skipped_tables)+1}次尝试错误" if table_not_found else f"错误"
